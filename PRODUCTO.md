...
| `nombre` en `empresas` es `UNIQUE` case-sensitive en SQLite, pero el importador matchea por `nombre.lower()`: nombres con distinto casing ("ACME" vs "acme") generan **dos registros separados** en la DB (cada uno con su propio país), mientras el importador los trata como la misma empresa al asignar cotizaciones nuevas → cotizaciones se cuelgan de uno solo, el otro queda como duplicado a la espera de fusión. **Esto NO es un bug a nivel DB**: ya existe `/api/limpiar/preview` + `/api/limpiar/unificar` (fuzzy matching + fusión con revisión humana, base de Sprint H), testeado en `TestLimpiezaUnificacion`, que está pensado exactamente para detectar y fusionar este tipo de duplicado. **Se intentó agregar `UNIQUE COLLATE NOCASE` a nivel DB y se revirtió**: bloquea la creación intencional de estos duplicados, rompiendo `test_preview_encuentra_grupos` y `test_unificar_fusiona_hacia_destino`. La solución correcta no es a nivel constraint, sino asegurar que el flujo de importación masiva termine derivando estos casos al preview de unificación en vez de dejarlos sueltos sin que nadie los revise | Media | Sprint H — integrar el flujo de importación con el preview/unificación existente, no tocar el constraint de `nombre` |
